...
func printParseTree(tokens)
	...
	see "                                       ____|_____" +nl
        ...
        see "                               key <key>  =  <movement> ;" + nl
	see "                                     |            |" + nl
        see "                                     "+ keyParts[2] + "          "+ actionParts[2] + nl 
    ok
    if keyCount = 3
        see "                         <program>" + nl
        see "                             |" + nl
	...
	see "                                                    _____|____"  + nl
        ...
        see "                                             key <key>  =  <movement> ;" + nl
        see "                                                  |            |" + nl    
        see "                                                  " + keyParts[3] + "          "+ actionParts[3] + nl 
    ok
    if keyCount = 4
        see "                         <program>" + nl
        see "                             |" + nl
	...
	see "                                                                  _____|____"  + nl                  
        ...
        see "                                                           key <key>  =  <movement> ;" + nl          
        see "                                                                |            |" + nl        
        see "                                                                " + keyParts[4] + "          " + actionParts[4] + nl          
    ok	
...
func writeToFile(tokens)
    ...
    fp = fopen("iZEBOT.BSP", "w")

    # Initialize set to track which movements are used
    usedMovements = []
    ...
    # Generate IF statements and track used movements
    currentKeyIndex = 2  # Start after 'wake'
    while currentKeyIndex < len(tokens) - 1  # Stop before 'sleep'
        if tokens[currentKeyIndex][:value] = "key"
            ...
            # Add routine to used movements if not already there
            if not find(usedMovements, routine)
                add(usedMovements, routine)
            ok
    ...
    # Add main loop closing and timeout handler
    code += "LOOP" + nl
    code += "Timeout:" + nl
    code += "GOSUB Motor_OFF" + nl
    code += "GOTO Main" + nl

    # Add movement procedures section header
    code += "'+++++ Movement Procedure +++++++++++++++++++++++++++++" + nl

    # Only add the subroutines that are actually used
    if find(usedMovements, "Forward")
        code += "Forward:" + nl
        code += "HIGH 13 : LOW 12 : HIGH 15 : LOW 14 : RETURN" + nl
    ok
    if find(usedMovements, "Backward")
        code += "Backward:" + nl
        code += "HIGH 12 : LOW 13 : HIGH 14 : LOW 15 : RETURN" + nl
    ok
    if find(usedMovements, "TurnLeft")
        code += "TurnLeft:" + nl
        code += "HIGH 13 : LOW 12 : LOW 15 : LOW 14 : RETURN" + nl
    ok
    if find(usedMovements, "TurnRight")
        code += "TurnRight:" + nl
        code += "LOW 13 : LOW 12 : HIGH 15 : LOW 14 : RETURN" + nl
    ok
    if find(usedMovements, "SpinLeft")
        code += "SpinLeft:" + nl
        code += "HIGH 13 : LOW 12 : HIGH 14 : LOW 15 : RETURN" + nl
    ok
    if find(usedMovements, "SpinRight")
        code += "SpinRight:" + nl
        code += "HIGH 12 : LOW 13 : HIGH 15 : LOW 14 : RETURN" + nl
    ok

    # Always include Motor_OFF as it's used in timeout handling
    code += "'++++++++++++++++++++++++++++++++++++++++++++++++++++++++" + nl
    code += "Motor_OFF:" + nl
